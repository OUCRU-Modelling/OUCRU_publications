...
```{r}
pubs22 <- read_excel("~/Desktop/2022 OUCRU Publications (Mar20).xlsx")
```

`pubs22` contains the data from the first sheet of the excel file:

```{r}
print(pubs22, width = 300)
```
...
This is basically what it looks like (again, showing here only the first few
publications):

```{r}
print(p22$data, width = 300)
```
